...
proc CyclicMMAPArr.dsiLocalSlice(ranges) {
  ...
  var arr = locArr(dom.dist.targetLocsIdx(low));
  var retarr = makeArrayFromPtr(arr.myElems, arr.locDom.myBlock.size:uint);
  return retarr;
}
...
class LocCyclicMMAPArr {
  ...
  // We are going to be changing the underlying high and low of the domain directly to avoid larger allocations
  var D = {0..1};
  // We are going to be changing the underlying ddata field to point to our mmap'd array.
  var A : [D] eltType;
  ...
  proc init(type eltType,
    ...
    if this.locDom.myBlock.size == 0 {
      ...
      myFName = mmapFilePrefix + "tester-" + uniqueFileSuffix.fetchAdd(1):string + "-" +  here:string + ".do-not-checkpoint";
      myFD = open(myFName.c_str(), O_RDWR);
      assert(myFD >= 0, "Failed to open: '", myFName, "'");
      var ret = ftruncate(myFD, size); // Make file exact requested size
      assert(ret == 0, "Unable to truncate file: '", myFName, "' to size of ", size, " bytes!");
      this.complete();
      var retptr = myElems:c_void_ptr;
      ret = sys_mmap(nil, size:uint, (PROT_READ | PROT_WRITE):int(32), MAP_SHARED:int(32), myFD, 0, retptr);
  ...
  proc dsiReallocate(bounds:rank*range) {
    if myElems != nil then munmap(myElems, this.locDom.myBlock.size * c_sizeof(eltType):int);
    if myFD != -1 then close(myFD);
    var oldSize = this.locDom.myBlock.size * c_sizeof(eltType):int;
    this.locDom.myBlock = bounds;
    var size = this.locDom.myBlock.size * c_sizeof(eltType):int;
    myFD = open(myFName.c_str(), O_RDWR);
    assert(myFD != -1, "Failed to open: '", myFName, "'");

    use Time;

    writeln("Reallocating ", myFName, " from ", oldSize, " to ", size);
    if size > oldSize {
      var ret = fallocate(myFD, FALLOC_FL_ZERO_RANGE, oldSize, size - oldSize); // Make file exact requested size
