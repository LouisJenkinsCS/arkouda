...
module SymArrayDmap
{
     ...
     */
    enum Dmap {defaultRectangular, blockDist, cyclicDist, cyclicMMAPDist, mmapRectangular};

    private param defaultDmap = if CHPL_COMM == "none" then Dmap.mmapRectangular
                                                       else Dmap.mmapRectangular; // Need to be cyclicMMAPDist
    /*
    ...
    */
    proc makeDistDom(size:int) {
        select MyDmap
        {
            ...
            when Dmap.mmapRectangular {
                return {0..#size} dmapped MMAPDist();
            }
            otherwise {
