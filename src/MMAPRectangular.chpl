...
config param debugMMAPDist = false;
config param debugMMAPDistBulkTransfer = false;
...
config param MMAPRectangularSupportsAutoLocalAccess = true;

enum ArrayStorageOrder { RMO, CMO }
config param defaultStorageOrder = ArrayStorageOrder.RMO;
// would like to move this into MMAPRectangularArr and
// MMAPRectangularDom so each instance can choose individually
param storageOrder = defaultStorageOrder;
  
class MMAPDist: BaseDist {
  override proc dsiNewRectangularDom(param rank: int, type idxType,
                                      param stridable: bool, inds) {
    const dom = new unmanaged MMAPRectangularDom(rank, idxType, stridable,
                                                    _to_unmanaged(this));
  ...
  proc dsiEqualDMaps(d:unmanaged MMAPDist) param return true;
...
class MMAPRectangularDom: BaseRectangularDom {
  var dist: unmanaged MMAPDist;
  ...
  proc type chpl__deserialize(data) {
    return MMAPDist.newRectangularDom(data.rank,
                                          data.idxType,
                                          data.stridable,
                                          data.dims);
...
*/
  ...
  proc dsiBuildArray(type eltType, param initElts:bool) {
    return new unmanaged MMAPRectangularArr(eltType=eltType, rank=rank,
                                                idxType=idxType,
                                                stridable=stridable,
  ...
  proc dsiBuildArrayWith(type eltType, data:_ddata(eltType), allocSize:int) {
    ...
    return new unmanaged MMAPRectangularArr(eltType=eltType,
                                      rank=rank,
                                      idxType=idxType,
                                      stridable=stridable,
...
//
// Copied from MMAPRectangularArr.getDataIndex
//
...
class MMAPRectangularArr: BaseRectangularArr {
  /*type eltType;
  param rank : int;
  type idxType;
  param stridable: bool;*/
  ...
  var dom : unmanaged MMAPRectangularDom(rank=rank, idxType=idxType,
                                          stridable=stridable);
  var off: rank*idxType;
  var blk: rank*int;
  ...
  override proc dsiDestroyArr(deinitElts:bool) {
    ...
    if (externArr) {
      ...
      munmap(data:c_void_ptr, numElts * c_sizeof(eltType):int);
      unlink(fname.c_str());
      close(fd);
    }
  }
  ...
  proc deinit() {
    if data != nil {
      munmap(data:c_void_ptr, dom.dsiNumIndices * c_sizeof(eltType):int);
      unlink(fname.c_str());
      close(fd);
    }
  }
  ...
  proc setupFieldsAndAllocate(param initElts) {
    ...
    var retptr = data:c_void_ptr;
    ret = sys_mmap(nil, (size * c_sizeof(eltType):int):uint, (PROT_READ | PROT_WRITE):int(32), MAP_SHARED:int(32), fd, 0, retptr);
    data = retptr:_ddata(eltType);
    assert(ret == 0, "Failed to mmap file: '", fname, "' with error: ", ret);
    if initElts {
  ...
  proc adjustBlkOffStrForNewDomain(d: unmanaged MMAPRectangularDom,
                                    alias: unmanaged MMAPRectangularArr)
  {
    for param i in 0..rank-1 {
      var s: idxType;
  ...
  override proc dsiReallocate(bounds: rank*range(idxType,
    ...
    if !isDefaultInitializable(eltType) {
      ...
      if (!disableArrRealloc && rank == 1 &&
        ...
        // _ddata reallocation for MMAP
        if data != nil then munmap(data:c_void_ptr, oldSize * c_sizeof(eltType):int);
        use Time;

        writeln("Reallocating ", fname:string, " from ", oldSize, " to ", newSize);
        if newSize > oldSize {
          var ret = fallocate(fd, FALLOC_FL_ZERO_RANGE, oldSize * c_sizeof(eltType):int, (newSize - oldSize) * c_sizeof(eltType):int); // Make file exact requested size
          assert(ret == 0, "Unable to fallocate file: '", fname:string, "' to size of ", newSize, " bytes! Error: ", ret);
          var retptr = data:c_void_ptr;
          ret = sys_mmap(nil, (newSize * c_sizeof(eltType):int):uint, (PROT_READ | PROT_WRITE):int(32), MAP_SHARED:int(32), fd, 0, retptr);
          data = retptr:_ddata(eltType);
          assert(ret == 0, "Failed to mmap file: '", fname, "' with error: ", ret);
          init_elts(data, newSize, eltType, lo=oldSize);
        } else {
          var ret = ftruncate(fd, newSize * c_sizeof(eltType):int); // Make file exact requested size
          assert(ret == 0, "Unable to truncate file: '", fname, "' from size of ", oldSize, " bytes to ", newSize, " bytes! Error: ", ret);
          var retptr = data:c_void_ptr;
          ret = sys_mmap(nil, (newSize * c_sizeof(eltType):int):uint, (PROT_READ | PROT_WRITE):int(32), MAP_SHARED:int(32), fd, 0, retptr);
          data = retptr:_ddata(eltType);
          assert(ret == 0, "Failed to mmap file: '", fname, "' with error: ", ret);
        ...
        halt("Error: dsiReallocate() not implemented for non-1D arrays");
      }
    }
  }
...
// call is more visible (e.g. ArrayViewSlice uses MMAPRectangular
...
proc chpl_serialReadWriteRectangularHelper(f, arr, dom) throws {
  ...
  proc recursiveArrayWriter(in idx: rank*idxType, dim=0, in last=false) throws {
    ...
    if dim == rank-1 {
      var first = true;
      if debugMMAPDist && f.writing then f.writeln(dom.dsiDim(dim));
...
proc _canDoSimpleTransfer(A, aView, B, bView) {
  if !A.isDataContiguous(aView._value) || !B.isDataContiguous(bView._value) {
    if debugMMAPDistBulkTransfer then
...
proc _canDoComplexTransfer(A, aView, B, bView) {
  return useBulkTransferStride;
}


proc transferHelper(A, aView, B, bView) : bool {
  if A.rank == B.rank &&
      (aView.stridable == false && bView.stridable == false) &&
      _canDoSimpleTransfer(A, aView, B, bView) {
    if debugMMAPDistBulkTransfer then
      chpl_debug_writeln("Performing simple MMAPRectangular transfer");

    _simpleTransfer(A, aView, B, bView);
  } else if _canDoComplexTransfer(A, aView, B, bView) {
    if debugMMAPDistBulkTransfer then
      chpl_debug_writeln("Performing complex MMAPRectangular transfer");
...
proc _simpleTransfer(A, aView, B, bView) {
  param rank     = A.rank;
  ...
  if debugBulkTransfer {
    ...
    chpl_debug_writeln("In MMAPRectangular._simpleTransfer():",
            " Alo=", Alo, ", Blo=", Blo,
...
proc _simpleParallelTransferHelper(A, B, Adata, Bdata, Alocid, Blocid, len) {
  const numTasks = if __primitive("task_get_serial") then 1
                      else _computeNumChunks(len);
  const lenPerTask = len:int/numTasks;

  if debugMMAPDistBulkTransfer && numTasks > 1 then
...
proc _simpleTransferHelper(A, B, Adata, Bdata, Alocid, Blocid, len, offset=0) {
...
*/
proc complexTransfer(A, aView, B, bView) {
  if (A.data.locale.id != here.id &&
      B.data.locale.id != here.id) {
    if debugMMAPDistBulkTransfer {
...
proc complexTransferCore(LHS, LViewDom, RHS, RViewDom) {
...
//
proc complexTransferComm(A, B, stridelevels:int(32), dstStride, srcStride, count, AFirst, BFirst) {
  if debugMMAPDistBulkTransfer {
...
proc MMAPRectangularDom.dsiSerialReadWrite(f /*: Reader or Writer*/) throws {
...
proc MMAPRectangularDom.doiToString() {
...
proc MMAPRectangularDom.dsiSerialWrite(f) throws { this.dsiSerialReadWrite(f); }
proc MMAPRectangularDom.dsiSerialRead(f) throws { this.dsiSerialReadWrite(f); }

proc MMAPRectangularArr.dsiSerialReadWrite(f /*: Reader or Writer*/) throws {
  chpl_serialReadWriteRectangular(f, this);
}

override proc MMAPRectangularDom.dsiSupportsAutoLocalAccess() param {
  return MMAPRectangularSupportsAutoLocalAccess;
}

proc MMAPRectangularArr.dsiSerialWrite(f) throws {
  dsiSerialReadWrite(f);
}

proc MMAPRectangularArr.dsiSerialRead(f) throws {
  dsiSerialReadWrite(f);
}

// This is very conservative.
proc MMAPRectangularArr.isDataContiguous(dom) {
  if debugMMAPDistBulkTransfer then
...
override proc MMAPRectangularArr.doiCanBulkTransferRankChange() param return true;

proc MMAPRectangularArr.doiBulkTransferToKnown(srcDom, destClass:MMAPRectangularArr, destDom) : bool {
  return transferHelper(destClass, destDom, this, srcDom);
}

proc MMAPRectangularArr.doiBulkTransferFromKnown(destDom, srcClass:MMAPRectangularArr, srcDom) : bool {
  return transferHelper(this, destDom, srcClass, srcDom);
}


/* This computes a 1D scan in parallel on the array, for 1D arrays only */
proc MMAPRectangularArr.doiScan(op, dom) where (rank == 1) &&
...
proc MMAPRectangularArr.doiOptimizedSwap(other: this.type) {
  // Get shape of array
  var size1: rank*int;
  ...
  if(this.locale == other.locale &&
    ...
    if debugOptimizedSwap {
      writeln("MMAPRectangular doing optimized swap. Domains: ", 
  ...
  if debugOptimizedSwap {
    writeln("MMAPRectangular doing unoptimized swap. Domains: ", 
            this.dom.ranges, " ", other.dom.ranges);
  }
  return false;
}
...
// clause.
proc MMAPRectangularArr.doiOptimizedSwap(other) where debugOptimizedSwap {
  writeln("MMAPRectangularArr doing unoptimized swap. Type mismatch");
  return false;
}
...
proc MMAPRectangularArr.chpl__preScan(op, res: [] ?resType, dom) {
  import RangeChunk;
...
proc MMAPRectangularArr.chpl__postScan(op, res, numTasks, rngs, state) {
  coforall tid in rngs.domain {
